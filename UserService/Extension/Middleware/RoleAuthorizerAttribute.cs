...
namespace UserServices.Extension.Middleware
{
    public class RoleAuthorizerAttribute : Microsoft.AspNetCore.Mvc.Filters.IAuthorizationFilter
    {
        ...
        public RoleAuthorizerAttribute(UserDbContext context, params string[] roles)
        {
            allowedRoles = roles;
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            if (tokenUser.Result.IsSuccess)
            {
                ...
                var allRoles = _context.UserProfileRoles.FirstOrDefault(x=>x.UserId == tokenUser.Result.Result.UserId);
                if (allRoles != null)
                {
                    var role = _context.UserRoles.Where(x => x.Id == allRoles.RoleId).Select(x=>x.Name).ToList();
