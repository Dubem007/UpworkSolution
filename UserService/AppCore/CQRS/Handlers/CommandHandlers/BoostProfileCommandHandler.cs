...
{
    public class BoostProfileCommandHandler : IRequestHandler<BoostProfileCommand, GenResponse<string>>
    {
        private readonly ILogger<BoostProfileCommandHandler> logger;
        private readonly IProfileRepository _profileRepo;
        private readonly IMediator mediator;

        public BoostProfileCommandHandler(ILogger<BoostProfileCommandHandler> logger, IProfileRepository profileRepo, IMediator mediator)
        {
            this.logger = logger;
        ...
        public async Task<GenResponse<string>> Handle(BoostProfileCommand request, CancellationToken cancellationToken)
        {
            GenResponse<string> objResp = new();
            try
            {
                var resp = await _profileRepo.BoostProfile(request.Entity);
