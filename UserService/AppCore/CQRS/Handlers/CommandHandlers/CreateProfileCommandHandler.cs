...
{
    public class CreateProfileCommandHandler : IRequestHandler<CreateProfileCommand, GenResponse<string>>
    {
        private readonly ILogger<CreateProfileCommandHandler> logger;
        private readonly IProfileRepository _profileRepo;
        private readonly IMediator mediator;

        public CreateProfileCommandHandler(ILogger<CreateProfileCommandHandler> logger, IProfileRepository profileRepo, IMediator mediator)
        {
            this.logger = logger;
        ...
        public async Task<GenResponse<string>> Handle(CreateProfileCommand request, CancellationToken cancellationToken)
        {
            GenResponse<string> objResp = new();
            try
            {
                var resp = await _profileRepo.CreateUserProfile(request.Entity);
